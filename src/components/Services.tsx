...
const Services = () => {
  ...
  const services = [
    ...
    },
    {
      ...
      description: 'Building secure, decentralized applications and smart contracts on blockchain technology for the future of digital transactions.',
      ...
      <div className="container mx-auto px-4 sm:px-6 pt-28 sm:pt-40 pb-20 sm:pb-32 relative z-10">
        ...
        <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-7xl mx-auto">
          {services.map((service, index) => (
            ...
            >
              <div className={`bg-white/80 backdrop-blur-sm dark:bg-[#2a2a2a]/80 rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-xl transition-all duration-500 cursor-pointer border-2 relative overflow-hidden ${
                ...
                <div className="relative z-10">
                  <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 transition-all duration-300 ${
                    ...
                    <service.icon className={`w-6 h-6 sm:w-8 sm:h-8 transition-colors duration-300 ${
                      hoveredService === index ? 'text-white' : 'text-gray-600 dark:text-gray-300 group-hover:text-red-600'
                    }`} />
                  </div>
                    ...
                    <ul className="space-y-2 sm:space-y-3 pt-2">
                      {service.features.map((feature, featureIndex) => (
                        <li key={featureIndex} className="flex items-center text-xs sm:text-sm text-gray-700 dark:text-gray-300 transition-colors duration-300">
                          <div className={`w-2 h-2 rounded-full mr-3 flex-shrink-0 transition-colors duration-300 ${
                            hoveredService === index ? 'bg-red-500' : 'bg-gray-400'
                          }`}></div>
                          {feature}
                        </li>
                      ))}
